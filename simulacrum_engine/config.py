...
DEFAULT_CONFIG = Path(APPDATA, "Simulacrum Engine")
...
class Config:
    ...
    def __init__(
        self,
        config_path: Path | str = Path(APPDATA, "simulacrum"),
    ...
    def save(self) -> None:
        ...
        for key, value in self.asset.model_dump().items():
            asset.add(key, value)
    ...
    def load(self) -> None:
        with open(self.config_file, "r") as file:
            ...
            self.asset_opts = self.config["asset"].unwrap()
    ...
    def load_asset(self) -> None:
        self.asset = AssetConfig(**self.asset_opts)
